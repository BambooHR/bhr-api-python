...
# --apply on minor bump updates pyproject.toml
TMPDIR_APPLY="$(mktemp -d /tmp/test_apply_XXXXXX)"
cat > "$TMPDIR_APPLY/pyproject.toml" <<'EOF'
[project]
name = "bamboohr-sdk"
version = "1.2.3"
EOF

OUTPUT="$(run_classify "$FIXTURES/additive.json" 0 --apply --version-file "$TMPDIR_APPLY/pyproject.toml")"
...
NEW_VER="$(grep '^version = ' "$TMPDIR_APPLY/pyproject.toml" | sed 's/version = "\(.*\)"/\1/')"
if [[ "$NEW_VER" == "1.3.0" ]]; then
    echo "  PASS: --apply minor writes 1.3.0 to pyproject.toml"
    PASS=$((PASS + 1))
else
    echo "  FAIL: --apply minor expected 1.3.0 in pyproject.toml, got: $NEW_VER"
    FAIL=$((FAIL + 1))
fi

# --apply on patch bump updates pyproject.toml
cat > "$TMPDIR_APPLY/pyproject.toml" <<'EOF'
[project]
name = "bamboohr-sdk"
version = "2.0.0"
...
OUTPUT="$(run_classify "$FIXTURES/metadata.json" 0 --apply --version-file "$TMPDIR_APPLY/pyproject.toml")"
...
if [[ "$NEW_VER" == "2.0.1" ]]; then
    echo "  PASS: --apply patch writes 2.0.1 to pyproject.toml"
    PASS=$((PASS + 1))
else
    echo "  FAIL: --apply patch expected 2.0.1 in pyproject.toml, got: $NEW_VER"
    FAIL=$((FAIL + 1))
fi
...
OUTPUT="$(run_classify "$FIXTURES/breaking.json" 1 --apply --version-file "$TMPDIR_APPLY/pyproject.toml")"
...
if [[ "$NEW_VER" == "2.0.0" ]]; then
    echo "  PASS: --apply major writes 2.0.0 to pyproject.toml"
    PASS=$((PASS + 1))
else
    echo "  FAIL: --apply major expected 2.0.0 in pyproject.toml, got: $NEW_VER"
    FAIL=$((FAIL + 1))
fi
...
run_classify "$FIXTURES/empty.json" 0 --apply --version-file "$TMPDIR_APPLY/pyproject.toml" > /dev/null
UNCHANGED_VER="$(grep '^version = ' "$TMPDIR_APPLY/pyproject.toml" | sed 's/version = "\(.*\)"/\1/')"
if [[ "$UNCHANGED_VER" == "3.1.4" ]]; then
    echo "  PASS: --apply none does not modify pyproject.toml"
    PASS=$((PASS + 1))
else
    echo "  FAIL: --apply none should not modify pyproject.toml, got: $UNCHANGED_VER"
    FAIL=$((FAIL + 1))
fi
...
# Unreadable/missing version in TOML produces a friendly error, not a silent crash
TMPDIR_ERR="$(mktemp -d /tmp/test_err_XXXXXX)"
cat > "$TMPDIR_ERR/pyproject.toml" <<'EOF'
[project]
name = "bamboohr-sdk"
EOF
ERR_OUTPUT="$(run_classify "$FIXTURES/additive.json" 0 --apply --version-file "$TMPDIR_ERR/pyproject.toml" 2>&1)" || true
rm -rf "$TMPDIR_ERR"
if echo "$ERR_OUTPUT" | grep -qF "Could not extract version"; then
    echo "  PASS: missing TOML version produces friendly error message"
    PASS=$((PASS + 1))
else
    echo "  FAIL: missing TOML version should produce friendly error message"
    echo "    Full output:"
