...
class StateProvinceResponseSchema(BaseModel):
    """
    Response from `GET /api/v1/meta/provinces/{countryId}`. The `options` list is sorted by `label` (subdivision abbreviation).
    """

    options: list[StateProvinceSchema] | None = Field(
        default=None,
        description="Subdivisions for the requested `countryId`. Each object matches StateProvinceSchema; `label` is the subdivision abbreviation, `name` the full name, `iso` the ISO 3166-2 code.",
    )
    ...
    def from_dict(cls, obj: dict[str, Any] | None) -> Self | None:
        ...
        _obj = cls.model_validate(
            {
                "options": [StateProvinceSchema.from_dict(_item) for _item in obj["options"]]
